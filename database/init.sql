...
-- 插入預設寵物資料（只在表格為空時插入）
INSERT INTO pets (name, breed, age, gender, color, category, price, description, health, images) 
SELECT * FROM (VALUES
...
) AS tmp(name, breed, age, gender, color, category, price, description, health, images)
WHERE NOT EXISTS (SELECT 1 FROM pets LIMIT 1);

-- 插入預設客戶評價（只在表格為空時插入）
INSERT INTO testimonials (name, pet_type, rating, text, avatar, is_active) 
SELECT * FROM (VALUES
...
) AS tmp(name, pet_type, rating, text, avatar, is_active)
...
-- 插入預設管理員資料 (密碼: xiaojiji2024)（只在表格為空時插入）
INSERT INTO admins (username, password, email, is_active) 
SELECT * FROM (VALUES
    ('admin', 'xiaojiji2024', 'admin@xiaojiji.com', TRUE)
) AS tmp(username, password, email, is_active)
WHERE NOT EXISTS (SELECT 1 FROM admins LIMIT 1);

-- 插入預設相簿資料（只在表格為空時插入）
INSERT INTO gallery_images (title, description, src, category, sort_order) 
SELECT * FROM (VALUES
...
) AS tmp(title, description, src, category, sort_order)
...
) AS tmp(title, content, type, is_active, start_date, end_date)
